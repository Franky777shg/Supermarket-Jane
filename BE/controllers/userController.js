...
// NOTE impor ttransporter nodemailer
...
module.exports = {
    ...
    register: async(req, res) => {
        ...
        try {
            ...
            const queryReg = `insert into users (username, email, password, role)
                            values (${db.escape(username)}, ${db.escape(email)}, ${db.escape(hashpass.toString())}, "user")`

            const resultReg = await asyncQuery(queryReg)

            const queryProfile= `insert into profile (id_users) values (${resultReg.insertId})`
            ...
            // NOTE send email notification to user
            const option = {
                from: `admin <kolakpisangsusu@gmail.com>`,
                to: `jane.dwptrdjaja@gmail.com`,
                subject: `E-Mail Verification`,
                text: `Hello, This is email verification trial using nodemailer`,
            }

            // NOTE set up handlebars
            const emailHandle= fs.readFileSync('./email/index.html').toString() // INI BNTKNYA BUFFER makanya di tostring

            // NOTE compile data emial
            const template= handlebars.compile(emailHandle)

            // NOTE menambahkan properti html didalam option
            // NOTE token nya ditaro dlm template supaya dikirim ke html untuk nampilin token di html
            option.html= template({ token: token, name: username })

            // NOTE untuk send email
